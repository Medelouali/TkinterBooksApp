...
from ui.logic.widget import widget, widgets, deleteWidgets
import ui.register.logging.logging as log
import tkinter.font as tkFont



def register(fr, last=[], err='', page="log", first=False):
    global lab
    lab=Label(fr, text="")
    deleteWidgets(last)
    ...
    if(page=="signUp"):
        ...
        signUp=Button(fr, text="Create Account", font=smallFont,
            command=lambda: registering(fr, widgets([signIn , *widgets_t, lab]), nam.get(), ema.get(), pas.get()))
        ...
        signIn = Button(fr, text="Already Have Account?", font=smallFont,
            command=lambda : commander(fr, widgets([*widgets_t, signIn, lab]), err, 'signIn'))
        ...
        username=Label(fr, text="Email", width=50, font=smallFont)
        username.grid(row=0, column=0)
        usernameE=Entry(fr, width=uiAttr["width"], textvariable=ema, font=smallFont)
        usernameE.grid(row=1, column=0, pady=uiAttr["pady"])
        ...
        logIn=Button(fr, text="Log In", 
            command=lambda: log.logging(fr, widgets([ signUp, *widgets_t, lab]), ema.get(), pas.get()))
        ...
        signUp = Button(fr, text="Don't Have An Account?", font=smallFont,
                        command=lambda : commander(fr, widgets([signUp, *widgets_t, lab ]), err, 'signUp'))
        ...
        submit=Button(frame, text="Log In", padx=1, pady=2, font=smallFont,
            command=lambda: log.logging(fr, widgets([ submit, *widgets_t, lab]), '', ''))
        ...
        submit=Button(frame, text="New Password", font=smallFont, 
            command=lambda: commander(fr, widgets([submit, *widgets_t, lab]), err, 'newPassword'))
        submit.grid(row=4, column=0, padx= 5)
    else:
        signIn=Button(fr, text="Sign In", font=smallFont, 
            command=lambda: commander(fr, widgets([signIn, signUp, forgotPassword, lab]), err, 'signIn'))
        signIn.grid(row=0, column=0, padx=40, pady=20)
        signUp=Button(fr, text="Sign Up", font=smallFont,
            command=lambda: commander(fr, widgets([signUp, signIn, forgotPassword, lab]), err, 'signUp'))
        ...
        forgotPassword=Button(fr, text="Forgot Password", font=smallFont, borderwidth=0, fg='blue',
            command=lambda: commander(fr, widgets([signUp, signIn, forgotPassword, lab]), err, 'forgot'))
        forgotPassword.grid(row=2, column=0, padx=40, pady=20)
    if(err):
        lab=Label(fr, text=err)
        lab.grid(row=20, column=0, columnspan=6)
