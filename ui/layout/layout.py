...
def layout(rt, reg={}, history=[]):
    ...
    books=Button(frame, text="Books", command=lambda: switchPage("books"), font=smallFont,
        width=labels["width"], height=labels["height"])
    books.grid(row=0, column=0, padx= 5)

    authors=Button(frame, text="Authors", command=lambda: switchPage("authors"), font=smallFont, 
        width=labels["width"], height=labels["height"])
    authors.grid(row=0, column=1, padx= 5)

    copies=Button(frame, text="Copies", command=lambda: switchPage("copies"), font=smallFont,
        width=labels["width"], height=labels["height"])
    copies.grid(row=0, column=2, padx= 5)

    editors=Button(frame, text="Editors", command=lambda: switchPage("editors"), font=smallFont,
        width=labels["width"], height=labels["height"])
    editors.grid(row=0, column=3, padx= 5)

    #the correspondant page should be displayed, this for setting the default page
    browser(frame, currentPage)
    widgets=[books, authors, copies, editors]
    getInTouch=Button(frame, text="Get In Touch", width=labels["width"]*2, height=1, font=smallFont,
        command=lambda: contactUs(rt, [*widgets, getInTouch]))
    getInTouch.grid(row=2, column=0, columnspan=4)
